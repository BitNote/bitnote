...
unsigned int nTargetSpacing = 2 * 60; // 2 minute block time
unsigned int nStakeMinAge = 8 * 60 * 60; // minimum age for coin age - 8 hours
...
const string strMessageMagic = "Bitnote Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	else if(nHeight >= 2 && nHeight <= 60000) {
	nSubsidy = 50 * COIN;
    }
...
const int DAILY_BLOCKCOUNT =  720;
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 3000)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bitnote", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "European election fall-out, storms and dementia";
        CTransaction txNew;
        txNew.nTime = 1401182152;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x66e4bc7d62d22c2a9d172b6cf4149db1d8ec360fa7e3b94cda746c16147ae7ac"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xda, 0xf1, 0xc6, 0xb0 }; // bitnote
